...
function carregarTabelas(filtroBusca = '', filtroCategoria = '') {
  ...
  const spanCrit   = $('#totalItensCriticos');
  const spanTotal  = $('#totalValorEstoque');

  if (tbEstoque)  tbEstoque.innerHTML  = htmlEstoque;
  if (tbCriticos) tbCriticos.innerHTML = htmlCriticos;
  if (tbTop5)     tbTop5.innerHTML     = htmlTop5;
    ...
    <tr>
      ...
      <td class="text-center">
        <button class="btn btn-sm btn-success"
                onclick="simularVenda('${v.servico.replace(/'/g,"\\'")}')">
          Vender Serviço
        </button>
      </td>
    </tr>
...
function listeners(){
  ...
  const selectCat  = $('#selectFiltroCategoria');

  if (btnFiltrar && inputBusca && selectCat){
 ...
 document.body.addEventListener('click', e => {
  ...
  // Botões principais
  const btnSalvarItem   = $('#btnSalvarItem');
  const btnGerarPedido  = $('#btnGerarPedido');

  if (btnSalvarItem)  btnSalvarItem.addEventListener('click', adicionarNovoItem);
